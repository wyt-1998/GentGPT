...
def generate_list_of_objects(objects_list, saving_path):
    ...
    for name in objects_list:
        ...
        filePath = saving_path[:-1] + ' ply/' + name + '.ply'
        mesh.export(file_type="ply", file_obj= filePath)
        n += 1

		# convert ply file to fbx file, codes reference: https://github.com/assimp/assimp/issues/2176
        #load ply file
        ply = pyassimp.load(filePath)
        #export as fbx file
        pyassimp.export(ply, saving_path + name + '.fbx', file_type='fbx')
